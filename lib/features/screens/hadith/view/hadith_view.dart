import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:islamic_app/features/screens/hadith/data/hadith_model.dart';
import '../../../../componant/app_container.dart';
import '../../../../componant/app_text.dart';
import '../../../../componant/custom_header.dart';
import '../controller/hadith_controller.dart';
import '../../../../utils/constant/app_strings.dart';

class HadithView extends StatefulWidget {
  const HadithView({super.key});

  @override
  State<HadithView> createState() => _HadithViewState();
}

class _HadithViewState extends State<HadithView> {
  late final HadithController controller;

  @override
  void initState() {
    super.initState();
    controller = HadithController();
    controller.onInit();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
      ),
      body: Column(
        children: [
          const CustomHeader(
            title: AppString.hadithTitle,
          ),
          Expanded(
            child: Obx(() {
              if (controller.isLoading.value) {
                return const Center(child: CircularProgressIndicator());
              }

              if (controller.books.isEmpty) {
                return Center(child: AppText(AppString.noHadithFound));
              }

              return ListView.separated(
                padding: EdgeInsets.fromLTRB(16.w, 16.h, 16.w, 100.h),
                itemCount: controller.books.length,
                separatorBuilder: (context, index) => SizedBox(height: 12.h),
                itemBuilder: (context, index) {
                  final book = controller.books[index];
                  return HadithListCard(book: book);
                },
              );
            }),
          ),
        ],
      ),
    );
  }
}

class HadithListCard extends StatelessWidget {
  const HadithListCard({
    super.key,
    required this.book,
  });

  final HadithBook book;

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        Get.toNamed('/hadith-detail', arguments: {'book': book});
      },
      child: AppContainer(
        padding: EdgeInsets.all(20.w),
        decoration: BoxDecoration(
          color: Theme.of(context).cardColor,
          borderRadius: BorderRadius.circular(12.r),
          boxShadow: [
            BoxShadow(
              color: Colors.black.withOpacity(0.05),
              blurRadius: 8,
              offset: const Offset(0, 2),
            ),
          ],
        ),
        child: Row(
          children: [
            AppContainer(
              width: 60.w,
              height: 60.w,
              decoration: BoxDecoration(
                color: Theme.of(context)
                    .primaryColor
                    .withOpacity(0.1),
                borderRadius: BorderRadius.circular(12.r),
              ),
              child: Icon(
                Icons.menu_book,
                size: 32.sp,
                color: Theme.of(context).primaryColor,
              ),
            ),
            SizedBox(width: 16.w),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  AppText(
                    book.name,
                    style: Theme.of(context).textTheme.titleLarge,
                  ),
                  SizedBox(height: 4.h),
                  AppText(
                    book.writerName,
                    style: Theme.of(context)
                        .textTheme
                        .bodyMedium
                        ?.copyWith(
                          color: Theme.of(context).primaryColor,
                        ),
                  ),
                  SizedBox(height: 4.h),
                  AppText(
                    '${book.numberOfHadith} ${AppString.hadithCount}',
                    style: Theme.of(context).textTheme.bodySmall,
                  ),
                ],
              ),
            ),
            Icon(
              Icons.arrow_forward_ios,
              size: 20.sp,
              color: Theme.of(context).dividerColor,
            ),
          ],
        ),
      ),
    );
  }
}
