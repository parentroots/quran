...
class DataService {
  ...
  Future<List<Ayah>> fetchSurahAyahs(int surahNumber) async {
    try {
      // Fetch Arabic text
      final arabicResponse = await ApiService.get(
        '${ApiEndpoint.quranBase}${ApiEndpoint.surah}/$surahNumber',
      );

      // Fetch Bengali translation
      final bengaliResponse = await ApiService.get(
        '${ApiEndpoint.quranBase}${ApiEndpoint.surah}/$surahNumber/bn.bengali',
      );

      if (arabicResponse.isSuccess && bengaliResponse.isSuccess) {
        ...
        for (int i = 0; i < arabicAyahs.length; i++) {
          ...
          ayahs.add(Ayah(
            ...
            sajda:
                arabicData['sajda'] is bool ? (arabicData['sajda'] ? 1 : 0) : 0,
            translation: bengaliData['text'],
          ));
        }
        return ayahs;
      }
      throw Exception(arabicResponse.message);
    } catch (e) {
      // log error
      rethrow;
    }
